...
struct child_info {					// 내가 죽을 때 정보를 명시적으로 남겨서 부모가 후에 볼 수 있게하기 위함
	bool is_zombie; 				// 내가 죽으면 zombie true로 바꿔줄 것
	tid_t tid;						// 내(자식)의 tid
	int exit_status;				// 내(자식)가 exit()으로 죽을 때 인자로 전달받은 exit status
	struct list_elem elem_c;		// child_info를 list_elem을 사용해서 child_list(연결리스트)로 관리
	struct semaphore sema;			// 자식 죽음기다릴 때 사용할 sema
