...
process_fork (const char *name, struct intr_frame *if_) {
	...
	struct semaphore *birth_sema = (struct semaphore *)malloc(sizeof(struct semaphore));		// 출산 대기를 위한 Sema(fork가 모두 끝날 때까지 엄마가 얌전히 기다릴 수 있도록)
	struct passing_args *pa = (struct passing_arg *)malloc(sizeof(struct passing_args));		// 자식이 태어나고 엄마정보를 복제하는 __do_fork에서 사용할 인자들을 담은 구조체
	...
	sema_init(pa->birth_sema, 0);		// sema_down시 바로 waiters에서 기다리게 하기위해 0으로 sema_init
	...
	int exit_status_child;  // 복사과정이 잘 끝났는지 검사하기 위한 과정
...
__do_fork (void *aux) {
	...
	process_activate (current); // process를 실행가능하도록 바꿔줌
#ifdef VM
	...
	for (int i = FD_MIN; i < FD_MAX; i++) {  // 부모의 fd_array를 그대로 가져오기
	...
	current->current_file = file_duplicate(parent->current_file); // 부모가 실행중인 FILE : exec로 실행된 fd가 없는 현재 실행중인 *file

	process_init ();
	memcpy(&current->tf, &if_, sizeof (struct intr_frame));  // 이 정보가 그대로 쓰일 일은 없긴 함. 단지 부모와 완전 똑같이 만들어주기 위함
...
process_wait (tid_t child_tid) {
	...
	while(elem_zombie != list_end(&curr->child_list)) {	// child_tid 와 같은 tid의 child_info 를 찾음
		...
		if(zombie->tid == child_tid) {
			...
			list_remove(elem_zombie);	// 자식이 사용하던 child_info구조체를 연결리스트에서 빼버림
			free(zombie);				// 자식이 사용하던 child_info구조체 free
			return ret;					
		}
